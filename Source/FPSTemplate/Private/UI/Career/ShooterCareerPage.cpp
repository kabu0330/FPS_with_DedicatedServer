...
void UShooterCareerPage::OnRetrieveMatchStats(const FDSRetrieveMatchStatsResponse& RetrieveMatchStatsResponse)
{
	...
	if (RetrieveMatchStatsResponse.hightestStreak > 0  ) AchievementData.Emplace(ESpecialElimType::Streak, RetrieveMatchStatsResponse.hightestStreak);
	...
	if (RetrieveMatchStatsResponse.headShootElims > 0  ) AchievementData.Emplace(ESpecialElimType::Headshot, RetrieveMatchStatsResponse.headShootElims);

	check(SpecialElimData);

	for (const TPair<ESpecialElimType, int32>& Achievement : AchievementData)
	{
		const FString& CareerAchievementName = SpecialElimData->SpecialElimInfo.FindChecked(Achievement.Key).CareerPageAchievementName;
		UTexture2D* Icon = SpecialElimData->SpecialElimInfo.FindChecked(Achievement.Key).ElimIcon;
		...
		{
			CareerAchievement->SetAchievementText(CareerAchievementName, Achievement.Value);
			if (Icon)
			{
